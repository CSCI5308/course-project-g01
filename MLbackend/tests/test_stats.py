...
class TestOutputStatistics(unittest.TestCase):
    ...
    @patch("csv.writer")
    @patch(
        "MLbackend.src.statsAnalysis.calculateStats", side_effect=mock_calculateStats
    )
    def test_output_statistics_with_data(
        self, mock_calculateStats, mock_csv_writer, mock_open
    ):
        data = [10, 20, 30]
        metric = "test_metric"

        metric, count, mean, stdev = outputStatistics(
            1, data, metric, self.output_dir, self.mock_logger
        )
