...
@pytest.mark.parametrize(
    "batch_dates, pr_counts, expected_pr_count",
    [
        (
            [datetime.now()],
            [
...
def test_addPRCountCorrect(
    result_instance: Result,
    batch_dates: List[datetime],
    pr_counts: List[int],
    expected_pr_count: List[int],
) -> None:
    ...
    assert result_instance.pr_count == expected_pr_count
    result_instance.logger.info.assert_called_once_with(
        "All values of Result are being reset"
    )
...
def test_addPRCountFailsDueToLessBatchSize(
    result_instance: Result,
    batch_dates: List[datetime],
    pr_counts: List[Any],
) -> None:

    result_instance.logger.error.return_value = f"Mismatch between batch size of {len(batch_dates)} and pr count of {len(batch_dates) + 1}"
    result_instance.logger.info.return_value = "All values of Result are being reset"
    result_instance.addBatchDates(batch_dates)
    ...
    result_instance.logger.error.assert_called_once_with(
        f"Mismatch between batch size of {len(batch_dates)} and pr count of {len(batch_dates) + 1}"
    )
...
def test_addPRCommentCountFailsDueToLessBatchSize(
    result_instance: Result,
    batch_dates: List[datetime],
    pr_comment_counts: List[Any],
) -> None:

    result_instance.logger.error.return_value = f"Mismatch between batch size of {len(batch_dates)} and pr_comment counts of {len(batch_dates) + 1}"
    result_instance.logger.info.return_value = "All values of Result are being reset"
    result_instance.addBatchDates(batch_dates)
    ...
    result_instance.logger.error.assert_called_once_with(
        f"Mismatch between batch size of {len(batch_dates)} and pr_comment count of {len(batch_dates) + 1}"
    )
...
@pytest.mark.parametrize(
    "batch_dates, pr_comment_sentiment_positive_counts",
    [([datetime.now()], [5, 5]), ([datetime.now(), datetime.now()], [5, 1, 6])],
)
def test_addPRCommentSentimentPositiveCountFailsDueToLessBatchSize(
    result_instance: Result,
    batch_dates: List[datetime],
    pr_comment_sentiment_positive_counts: List[Any],
) -> None:

    result_instance.logger.error.return_value = f"Mismatch between batch size of {len(batch_dates)} and pr_comment_sentiment_positive count of {len(batch_dates) + 1}"
    result_instance.logger.info.return_value = "All values of Result are being reset"
    result_instance.addBatchDates(batch_dates)
    ...
    result_instance.logger.error.assert_called_once_with(
        f"Mismatch between batch size of {len(batch_dates)} and pr_comment_sentiment_positive count of {len(batch_dates) + 1}"
    )
...
def test_addPRCommentSentimentPositiveCountFailsDueToIncorrectPRCommentSentimentPositiveValueType(
    result_instance: Result,
    batch_dates: List[datetime],
    pr_comment_sentiment_positive_counts: List[Any],
) -> None:
...
@pytest.mark.parametrize(
    "batch_dates, pr_comment_sentiment_negative_counts, expected_pr_comment_sentiment_negative_count",
    [
        (
            [datetime.now()],
            [
...
def test_addPRCommentSentimentNegativeCountCorrect(
    result_instance: Result,
    batch_dates: List[datetime],
    pr_comment_sentiment_negative_counts: List[int],
    expected_pr_comment_sentiment_negative_count: List[int],
) -> None:

    result_instance.logger.info.return_value = "All values of Result are being reset"
    result_instance.addBatchDates(batch_dates)
    for idx, pr_comment_sentiment_negative_count in enumerate(
        pr_comment_sentiment_negative_counts
    ):
        result_instance.addPRCommentSentimentNegativeCount(
            batch_idx=idx,
            pr_comment_sentiment_negative_count=pr_comment_sentiment_negative_count,
        )

    assert (
        result_instance.pr_comment_sentiment_negative_count
        == expected_pr_comment_sentiment_negative_count
    )
    result_instance.logger.info.assert_called_once_with(
        "All values of Result are being reset"
    )
...
@pytest.mark.parametrize(
    "batch_dates, pr_comment_sentiment_negative_counts",
    [([datetime.now()], [5, 5]), ([datetime.now(), datetime.now()], [5, 1, 6])],
)
def test_addPRCommentSentimentNegativeCountFailsDueToLessBatchSize(
    result_instance: Result,
    batch_dates: List[datetime],
...
def test_addPRCommentSentimentNegativeCountFailsDueToIncorrectPRCommentSentimentNegativeValueType(
    result_instance: Result,
    batch_dates: List[datetime],
    pr_comment_sentiment_negative_counts: List[Any],
) -> None:
...
def test_addPRCommentToxicityPercentageFailsDueToLessBatchSize(
    ...
    result_instance.logger.error.return_value = f"Mismatch between batch size of {len(batch_dates)} and pr_comment_toxicity_percentage of {len(batch_dates) + 1}"
    result_instance.logger.info.return_value = "All values of Result are being reset"
    result_instance.addBatchDates(batch_dates)
    ...
    result_instance.logger.error.assert_called_once_with(
        f"Mismatch between batch size of {len(batch_dates)} and pr_comment_toxicity_percentage of {len(batch_dates) + 1}"
    )
